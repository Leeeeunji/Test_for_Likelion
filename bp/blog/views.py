...
def detail(request, blog_id):
    detail = get_object_or_404(Blog, pk=blog_id)        # 글을 가져오기 OR 에러가 나면 404 표기
    comments = Comment.objects.all().filter(post = detail)  # models.py 에 있던 변수 post 를 필터링해서 가져오기
    
    
    #likes => 순서쌍 ('현재 blog_id', '현재 user.id')
...
def comment_delete(request, comment_id):
    ...
    blog_id = comment.post.id                               #삭제한 이후 원래 사이트에 남아있기 위해 블로그 아이디 가져오기~~
    comment.delete()
